...
./build.sh
```
Once the compilation is complete, you'll find the compiled `.wasm` file of the contract at: `target/wasm32-unknown-unknown/release/poll.wasm`.
...
near deploy \
  ...
  --networkId "$new-calimero-testnet"
```
...
- **NEXT_PUBLIC_WALLET_URL**: Link to your NEAR wallet.
