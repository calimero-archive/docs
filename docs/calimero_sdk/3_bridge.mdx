...
### Deny Accounts from using XSC for specific Contracts

``` denyCrossShardCallPerContract ``` returns ``` true ``` if a rule is successfully added to the deny list of pairs of contract rules and account rules. ``` denyCrossShardCallPerContract ``` adds pair of account regex rule and contract regex rule, and if an account wants to use XSC for a contract and that pair matches at least one rule in the deny list, XSC can not be called for that combination. Note that if the account and contract pair does not match any of the rules in deny list, the account still needs to have valid permissions for using the XSC connector.
...
### Remove denied Accounts from using XSC for specific Contracts

``` removeDeniedCrossShardCallPerContract ``` returns ``` true ``` if deny pair is successfully removed from the list of rules, and ``` false ``` otherwise. This method removes rule pairs from the same list that ``` denyCrossShardCallPerContract ``` adds rules.
...
### Check if the Account can use the Contract through the XSC connector

``` canMakeCrossShardCallForContract ``` returns ``` true ``` if the Account can call Contract through the XSC connector, ``` false ``` otherwise.
...
### Get pairs of Account and Contract regex rules for using the XSC connector

``` getAccountPerContractDeniesForXsc ``` returns an array of pairs of deny regex rules for an XSC connector. Note that if there are more than 20 pairs this method will return exactly 20 rules and others will not be shown. Also, the order of the pairs is not guaranteed, and if there are more than 20 pairs arbitrarily 20 pairs are returned.
