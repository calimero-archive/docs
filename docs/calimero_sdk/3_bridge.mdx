...
``` ftBalanceOf ``` method returns the number of fungible tokens (on contract that can be both on Near and Calimero side) of a given account.
...
``` getNftOwnerId ``` returns a owner of the NFT on a given chain.
...
``` getNftTokensForOwner ``` returns all NFTs on a NFT contract for a given owner.

```js
const res = await bridge.getNftTokensForOwner(
    ChainType.NEAR,
    "paras-token-v2.testnet", // contract id
    "nft_connector.q.dev.calimero.testnet" // account id, here we are checking which tokens are locked on NFT Connector account on Near side
);
...
``` getCurrentBlockHeight ``` returns current (last verified) Light client block height on Calimero or Near side.
...
### Check if account can use bridge

``` canBridge ``` returns ``` true ``` if a given account can use defined bridge (FT, NFT or XSC), ``` false ``` otherwise.

```js
const res = await bridge.canBridge(
    ChainType.NEAR, // defines source chain for the checking, if this call returns true that means that account can use a defined bridge in the direction from Near to Calimero (note that this does not mean that it can bridge from Calimero to Near).
    "check.for.account.testnet", // account for which we are checking if it can use bridge
...
### Get all allow rules for a connector

``` getAllowRegexRules ``` returns an array of regex rules (passed as strings) for a given connector. Note that if there are more than 10 rules this method will return exactly 10 rules and others will not be shown. Also, order of the rules is not guaranteed, and if there are more than 10 rules arbitrary 10 rules are returned.
...
``` addAllowRegexRule ``` returns ``` true ``` if rule is successfully added to the list of rules, ``` false ``` otherwise. ``` addAllowRegexRule ``` adds a regex rule for allowing all accounts that match that rule to use a defined connector.
...
``` removeAllowedRegexRule ``` returns ``` true ``` if rule is successfully removed from the list of rules, ``` false ``` otherwise. ``` removeAllowedRegexRule ``` removes a regex rule from the allowing list of a connector.

```js
const connectorPrivateKey = "ed25519:connector-private-key"; // to remove rules from the connector permissions we need to provide a private key of a given connector. In this example that will be removing rule from the FT connector.
const connectorKeyPair = KeyPair.fromString(privateKey);
...
### Deny Accounts from using XSC for a specific Contracts

``` denyCrossShardCallPerContract ``` returns ``` true ``` if rule is successfully added to the deny list of pairs of contract rules and account rules. ``` denyCrossShardCallPerContract ``` adds pair of account regex rule and contract regex rule, and if account wants to use XSC for a contract and that pair matches at least one rule in the deny list, XSC can not be called for that combination. Note that if account and contract pair does not match any of the rules in deny list, account still needs to have valid permissions for using XSC connector.

```js
const privateKeyXsc = "ed25519:private-key-xsc-connector";
const keyPairXsc = KeyPair.fromString(privateKeyXsc);

const res = await bridge.denyCrossShardCallPerContract(
    ChainType.NEAR,
    keyPairXsc,
    ".*", // account regex rule
...
### Remove denied Accounts from using XSC for a specific Contracts

``` removeDeniedCrossShardCallPerContract ``` returns ``` true ``` if deny pair is successfully removed from the list of rules, ``` false ``` otherwise. This method removes rule pairs from the same list that ``` denyCrossShardCallPerContract ``` adds rules to.

```js
const privateKeyXsc = "ed25519:private-key-xsc-connector";
const keyPairXsc = KeyPair.fromString(privateKeyXsc);

res = await bridge.removeDeniedCrossShardCallPerContract(
    ChainType.NEAR,
    keyPairXsc,
    ".*",
...
### Get pairs of Account and Contract regex rules for using XSC connector

``` getAccountPerContractDeniesForXsc ``` returns an array of pairs of deny regex rules for a XSC connector. Note that if there are more than 20 pairs this method will return exactly 20 rules and others will not be shown. Also, order of the pairs is not guaranteed, and if there are more than 20 pairs arbitrary 20 pairs are returned.

```js
const res = await bridge.getAccountPerContractDeniesForXsc(ChainType.NEAR);
