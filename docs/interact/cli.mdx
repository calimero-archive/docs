...
Replace `<YOUR_SHARD_RPC_URL>`

 with the RPC URL of your Calimero shard.
...
Feel free to adjust the alias according to your preferences and requirements.

By following these steps, you can install the NEAR CLI, set up the necessary configuration, and interact with the Calimero shard using the NEAR CLI tool.
...
Apologies for the confusion. Here are the parameters that you can export:
...
```bash
near state $MAIN_ACCOUNT_ID --nodeUrl $SHARD_RPC_URL --networkId $SHARD_ID
```

By exporting these parameters, you can easily modify and reuse them in your terminal session or scripts.