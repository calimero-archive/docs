...
approvals_after_next contain valid signatures on approval_message from the block producers of the corresponding epoch (see next section);
The signatures present in approvals_after_next correspond to more than 2/3 of the total stake (see next section).
If next_bps is not none, sha256(borsh(next_bps)) corresponds to the next_bp_hash in inner_lite.

NEAR on ETH light client has optimistic validation since 1 signature verification on ETH costs 500-600k gas which is prohibitively expensive.

For Calimero on the NEAR light client there is a limitation as on NEAR there is a 300 Tgas limit  per transaction. Signature check ins are also expensive (with ~50 signature checks we break the limit and tx fails). However the good news is that initially the number of validators on Calimero will be limited to around 20, so this will pass.
...
For the NEAR on Calimero light client contract the signature verifying cost is not an issue
since the shard creator doesn’t pay gas fees on the Calimero Shard.
