...
,bridging will not work. If asset is not bridged in the following 96 hours from the initial locking transaction, asset will no longer be
...
the original owner (user). Note that if the admin unlocks it before 4 days pass user will effectively get his assets two times since he can
still prove that he locked assets (and bridge it to destination chain), and he will get locked assets on the source chain back as well.
