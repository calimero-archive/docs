---
title: Javascript Contract Development
sidebar_position: 5
---

JavaScript is the most popular programming language, used by nearly 14 million developers worldwide. 

You can write smart-contracts in JavaScript or Typescript, and our SDK will pack them with a lightweight runtime.

This can then be compiled into a single Wasm file that can be deployed into the NEAR network.

:::info
Calimero Javascript SDK code is similar to the NEAR Javascript SDK. For more information, you can refer the [NEAR docs](https://docs.near.org/sdk/js/introduction)
:::

## Create Your First Javascript Contract
Create your first **Javascript contract** in minutes:
1. Download and install [Node.js](https://nodejs.org/en/download/).
2. Create a new **javascript** project using our [quickstart guide](https://docs.near.org/develop/quickstart-guide).
3. Read our docs on **[how to write smart contract](https://docs.near.org/develop/contracts/anatomy)**.