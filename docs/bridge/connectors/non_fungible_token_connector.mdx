...
The Non-Fungible Token Connector consists of four contracts: `nft_connector`, `bridge_token_deployer`, `bridge_token`, and `permissions_manager`. These contracts need to be installed on both chains.

To install the bridge and fulfill all prerequisites, refer to the [Running a Bridge](https://docs.calimero.network/bridge/running_a_bridge) section.

In this description, we assume that a bridge between the NEAR Testnet and a Calimero shard is already set up.
