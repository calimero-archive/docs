...
The bridge service is a crucial component that facilitates token transfers and enables cross-shard calls between different blockchain networks. This section focuses on the transfer of NEAR tokens from the mainnet to a Calimero shard as an example use case.
