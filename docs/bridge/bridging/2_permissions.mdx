...
5. 30% of used gas (gas paid by Cross Shard Connector on the Near side) for method execution goes to `contract.testnet` (account under attacker's control) as a developer incentive. [Link to Near gas documentation](https://docs.near.org/concepts/basics/transactions/gas)
